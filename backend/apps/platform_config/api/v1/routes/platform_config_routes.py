...
from apps.platform_config.application.dto.scheduler_process_config_dto import (
    ScheduledProcessConfigDTO,
)
from apps.platform_config.application.dto.scheduler_process_config_update_dto import (
    ScheduledProcessConfigUpdateDTO,
)
...
def listar_scheduled_processes(
    enabled_only: bool = False, service: PlatformConfigService = Depends(get_service)
):
    return service.obtener_scheduled_process_configs(enabled_only=enabled_only)


@router.get(
    "/scheduled-processes/{process_name}", response_model=ScheduledProcessConfigDTO
)
def obtener_scheduled_process(
    process_name: str, service: PlatformConfigService = Depends(get_service)
):
    cfg = service.obtener_scheduled_process_config(process_name)
    if not cfg:
        raise HTTPException(
            status_code=404, detail="ScheduledProcessConfig no encontrado"
        )
    return cfg
...
def actualizar_scheduled_process(
    process_name: str,
    dto: ScheduledProcessConfigUpdateDTO,
    service: PlatformConfigService = Depends(get_service),
):
    try:
...
def crear_process(
    dto: ProcessCreateDTO, service: PlatformConfigService = Depends(get_service)
):
    return service.registrar_process(dto)


@router.get("/processes", response_model=List[ProcessDTO])
def listar_processes(
    active_only: bool = False, service: PlatformConfigService = Depends(get_service)
):
...
@router.post("/processes/{code}/pause", response_model=ProcessDTO)
def pausar_process(code: str, service: PlatformConfigService = Depends(get_service)):
    """Pausa un proceso específico (is_active=False)."""
    try:
        dto = ProcessUpdateDTO(is_active=False)
        return service.actualizar_process(code, dto)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))


@router.post("/processes/{code}/resume", response_model=ProcessDTO)
def reanudar_process(code: str, service: PlatformConfigService = Depends(get_service)):
    """Reanuda un proceso específico (is_active=True)."""
    try:
        dto = ProcessUpdateDTO(is_active=True)
        return service.actualizar_process(code, dto)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
